...
const BirdBreeds = () => {
  ...
  useEffect(() => {
    const fetchBirdBreeds = async () => {
      try {
        setLoading(true);
        // Bird type ID is 4
        const data = await getBreedsByPetType(4);
        setBirdBreeds(data);
        setError(null);
      } catch (err) {
        console.error('Error fetching bird breeds:', err);
        setError('Failed to load bird breeds. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <h1 className="text-2xl font-bold mb-6">Loading Bird Breeds...</h1>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Bird Breeds Available for Adoption</h1>
        ...
        {birdBreeds.map((breed) => (
          ...
          >
            <div className="h-64 bg-gray-200">
              <img 
                src={breed.image_url || `https://source.unsplash.com/300x200/?bird,${breed.name.replace(' ', '+')}`} 
                alt={breed.name}
