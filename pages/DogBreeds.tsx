...
const DogBreeds = () => {
  ...
  useEffect(() => {
    const fetchDogBreeds = async () => {
      try {
        setLoading(true);
        // Dog type ID is 1
        const data = await getBreedsByPetType(1);
        setDogBreeds(data);
        setError(null);
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <h1 className="text-2xl font-bold mb-6">Loading Dog Breeds...</h1>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Dog Breeds Available for Adoption</h1>
        ...
        {dogBreeds.map((breed) => (
          ...
          >
            <div className="h-64 bg-gray-200">
              <img 
                src={breed.image_url || `https://source.unsplash.com/300x200/?dog,${breed.name.replace(' ', '+')}`} 
                alt={breed.name}
